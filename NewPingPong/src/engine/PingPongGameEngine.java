...
MouseMotionListener, KeyListener, GameConstants{
	...
	private volatile boolean ballServed = false; // Mark variable as frequently changing (static), this helps ensure both threads access same state (for faster processors) 
	...
	public void mouseMoved(MouseEvent e) {
		int mouse_Y = e.getY();
		// If a mouse is above the kid's racket and the racket did not go over the
		// table top move it up, otherwise move it down
			...
			if(ballServed) { // if ball is moving
				...
				if ( !movingLeft && ballX <= BALL_MAX_X){
					...
					if (ballX >= KID_RACKET_X && canBounce){ 
						movingLeft=true; // Changed KID_RACKET_X_START
						verticalSlide=2 - ((ballY - kidRacket_Y) / (RACKET_LENGTH / 5));
	...
	private void kidServe(){
		ballServed = true;
		ballX = KID_RACKET_X - 10; // Changed KID_RACKET_X_START
		ballY=kidRacket_Y;
		if (ballY > TABLE_HEIGHT/2){ // If the ball is in the top half of the table
			verticalSlide=-1;
		}else{
			verticalSlide=1;
		}
